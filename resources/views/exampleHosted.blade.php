...
{{ session([
    ...
    <div class="row mt-5">
        <div class="col-md-8 order m-auto ">
            <form action="{{ url('/pay') }}" method="POST" class="needs-validation">
                ...
                <ul class="list-group mb-3">
                    ...
                    <li class="list-group-item d-flex justify-content-between">
                        <span>Total (BDT)</span>
                        <input type="hidden" name="total" value="{{ $data['total'] - ($data['total']*$data['discount'])/100 + ($data['delivery_charge'])}}">
                        <strong>{{ $data['total'] - ($data['total']*$data['discount'])/100 + ($data['delivery_charge'])}}</strong>
                    </li>
                </ul>
